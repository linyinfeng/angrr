...
in
{
  ...
  testScript = /* python */ ''
    ...
    machine.succeed("readlink /nix/var/nix/profiles/system-14-link") # keep (oldest of weekly [0, 7 days))
    machine.succeed("readlink /nix/var/nix/profiles/system-13-link") # keep (oldest of weekly [7, 14 days))
    machine.succeed("test ! -e /nix/var/nix/profiles/system-12-link") # delete (younger in weekly [14, 21 days))
    machine.succeed("readlink /nix/var/nix/profiles/system-11-link") # keep (oldest of weekly [14, 21 days))
    machine.succeed("readlink /nix/var/nix/profiles/system-10-link") # keep (oldest remaining of monthly [0, 30 days))
    machine.succeed("test ! -e /nix/var/nix/profiles/system-9-link")
    ...
    machine.succeed("readlink /nix/var/nix/profiles/system-4-link") # keep (oldest of monthly [30, 60 days))
    machine.succeed("test ! -e /nix/var/nix/profiles/system-3-link") # delete (past monthly coverage)
